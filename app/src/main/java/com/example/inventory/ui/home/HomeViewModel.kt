...
class HomeViewModel (usersRepository: UsersRepository) : ViewModel() {
    ...
    val homeUiState: StateFlow<HomeUiState> = usersRepository.getAllUsers()
        .map { HomeUiState(itemList = it) } // itemList (que é de users) pra estado de UI
        .stateIn(
            scope = viewModelScope, // corrotina
